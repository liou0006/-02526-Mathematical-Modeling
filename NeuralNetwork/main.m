...
global stopExecution; % Global variable to track stopping
stopExecution = false; % Initialize as false (continue execution)
...
% 8 Layers
rotatelist =     [90,   45,   -33,     0,    0,  0,  0, 0, 0]; % Rotation angles
biaslist =       [0,    -120,    -50,    -20,  0,  0, 0, 0]; % Bias values
activationList = [0,    0,    1,       1,    0,  0,  0, 0, 0]; % Activation function flags
...
for i = 1:numel(biaslist)
    ...
    if stopExecution % Check if user stopped execution
    ...
    controlFig = figure('Name', 'Continue or Stop?', 'NumberTitle', 'off', 'Position', [600, 400, 250, 100]);
...
uiresume(gcbf); % Resume execution to allow exit
end